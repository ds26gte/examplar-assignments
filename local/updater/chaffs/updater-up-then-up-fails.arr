...
#| chaff (tdelvecc, cli135, Sep 9, 2020): 
    Raises an error if up is immediately followed by an up.
...
fun up<A>(cur :: Cursor<A>) -> Cursor<A> block:
  ...
  # CHAFF DIFFERENCE: Raises an error if up directly follows an up.
  block:
    cases (List<Action>) cur.actions:
      | empty => nothing
      | link(f, _) =>
        if f == Up:
          raise("Up followed by up.")
        else:
          nothing
        end
    end
