...
#| chaff (tdelvecc, Sep 1, 2020): 
    Using down when not allowed returns input instead of erroring.
...
fun down<A>(cur :: Cursor<A>, child-index :: Number ) -> Cursor<A>:
  ...
  cases (Tree<A>) cur.sub-tree block:
    | mt => raise("Shouldn't reach here.")
    | node(_, children) =>
      # CHAFF DIFFERENCE: Returns given cursor instead of erroring.
      if (child-index < 0) or (child-index >= children.length()):
        cur
      else:
        cursor(
          some(cur),
