...
#| chaff (tdelvecc, Sep 1, 2020): 
    Raises an error if update is followed by another action.
...
fun up<A>(cur :: Cursor<A>) -> Cursor<A> block:
  doc: "Returns the parent cursor"
  
  # CHAFF DIFFERENCE: Raises an error if up directly follows an update.
  block:
    cases (List<Action>) cur.actions:
      | empty => nothing
      | link(f, _) =>
        if f == Update:
          raise("Update followed by up.")
        else:
          nothing
        end
    end
...
fun left<A>(cur :: Cursor<A>) -> Cursor<A> block:
  doc: "Returns a cursor pointed at the node to the left"
  
  # CHAFF DIFFERENCE: Raises an error if left directly follows an update.
  block:
    cases (List<Action>) cur.actions:
      | empty => nothing
      | link(f, _) =>
        if f == Update:
          raise("Update followed by left.")
        else:
          nothing
        end
    end
...
fun right<A>(cur :: Cursor<A>) -> Cursor<A> block:
  doc: "Returns a cursor pointed at the node to the right"
  
  # CHAFF DIFFERENCE: Raises an error if right directly follows an update.
  block:
    cases (List<Action>) cur.actions:
      | empty => nothing
      | link(f, _) =>
        if f == Update:
          raise("Update followed by right.")
        else:
          nothing
        end
    end
...
fun down<A>(cur :: Cursor<A>, child-index :: Number ) -> Cursor<A> block:
  doc: "Returns a cursor to a child node specified by the index argument"
  
  # CHAFF DIFFERENCE: Raises an error if down directly follows an update.
  block:
    cases (List<Action>) cur.actions:
      | empty => nothing
      | link(f, _) =>
        if f == Update:
          raise("Update followed by down.")
        else:
          nothing
        end
    end
...
fun update<A>(cur :: Cursor<A>, func :: (Tree<A> -> Tree<A>)) -> Cursor<A> block:
  doc: "Updates the cursor's sub-tree with the given tree function"
  
  # CHAFF DIFFERENCE: Raises an error if update directly follows an update.
  block:
    cases (List<Action>) cur.actions:
      | empty => nothing
      | link(f, _) =>
        if f == Update:
          raise("Update followed by update.")
        else:
          nothing
        end
    end
