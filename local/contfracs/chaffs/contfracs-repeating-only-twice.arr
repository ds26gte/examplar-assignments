...
#| chaff (tdelvecc, Sep 1, 2020): 
    repeating-stream(-opt) only repeats twice, then repeats first element infinitely
...
fun repeating-stream<A>(original :: List<A>) -> Stream<A> block:
  ...
  # CHAFF DIFFERENCE: Repeats list twice, then first element infinitely
  fun helper(remaining :: List<A>, count :: Number) -> Stream<A>:
    cases (List<A>) remaining:
      | empty => helper(original, count + 1)
      | link(f, r) => 
        if count > 1:
          lz-link(f, {(): helper(remaining, count)})
        else:
          lz-link(f, {(): helper(r, count)})
        end
    end
  end
