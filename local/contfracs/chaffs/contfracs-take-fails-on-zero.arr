...
#| chaff (tdelvecc, Sep 1, 2020): 
    take raises an error when 0 elements are taken.
...
fun take<A>(stream :: Stream<A>, n :: Number) -> List<A>:
  ...
  ask:
      # CHAFF DIFFERENCE: Raises an error when 0 elements are taken.
    | n > 1 then: link(lz-first(stream), take(lz-rest(stream), n - 1))
    | n == 1 then: link(lz-first(stream), empty)
    | n == 0 then: raise("Zero passed to take.")
