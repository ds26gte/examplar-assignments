...
#| chaff (tdelvecc, Sep 1, 2020): 
    fraction-stream-opt raises an error when no coefficients are available
...
fun fraction-stream-opt(stream :: Stream<Option<Number>>) -> Stream<Option<Number>>:
  ...
  fun helper(
    ...
    cases (Option<Number>) lz-first(suffix) block:
      | none =>
        # CHAFF DIFFERENCE: Raises error when first coeff is none.
        when (index == 0):
          raise("No coefficients provided.")
        end
        
        repeating-stream([list: none])
      | some(init-value) =>
