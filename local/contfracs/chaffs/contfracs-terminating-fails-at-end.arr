...
#| chaff (tdelvecc, Sep 1, 2020): 
    terminating-stream errors when nones are reached.
...
fun terminating-stream<A>(lis :: List<A>) -> Stream<Option<A>>:
  ...
  cases (List<A>) lis:
      # CHAFF DIFFERENCE: Raises error when the stream runs out of elements.
    | empty => raise("terminating-stream received empty.")
