...
#| wheat (tdelvecc, Sep 6, 2020): 
    - age always 0
    - name always ""
    - returns empty when n < 0 for generate-input
    - allows invalid people for is-valid
    - uses good generate-input for oracle
...
fun generate-input(n :: Number) -> List<Person> block:
  ...
  if n <= 0:
    # WHEAT DIFFERENCE: returns empty when n < 0
    empty
  else:
    for lysts.map(_ from range(0, n)):
      # WHEAT DIFFERENCE: age always 0
      # WHEAT DIFFERENCE: name always ""
      person("", 0)
    end
  end
end
...
fun generate-input-good(n :: Number) -> List<Person> block:
  ...
  for lysts.map(_ from range(0, n)):
    person(
      ...
      num-random(1e6) + 1e6 + (num-random(100) / 100))
  end
end
...
fun oracle(sorter :: (List<Person> -> List<Person>)) -> Boolean:
  ...
  interesting-inputs :: List<List<Person>> = [list:
    ...
    lysts.map2({(p, age): person(p.name, age)}, generate-input-good(150), range(0, 150))
      .reverse()
  ]
